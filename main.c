...
void funcao_run_normal(char *argumentos[], int contador_de_argumentos, Tarefas cadastradas[], int quantidade_de_tarefas){
    ...
    if(indice == -1){
        printf("Erro: Tarefas nao existe.\n");
        return;
    }
    ...
    for(int i = 0; i < cadastradas[indice].quantidade_de_argumentos; i++){
    ...
    if(pid < 0){
        ...
        int fd_in = open(cadastradas[indice].input_do_arquivo, O_RDONLY);
        ...
        if(cadastradas[indice].estado_do_output == 1){
            int fd_out = open(cadastradas[indice].output_doarquivo, O_WRONLY | O_CREAT | O_TRUNC, 0644);
            if(fd_out < 0){
            ...
            int fd_out = open(cadastradas[indice].output_doarquivo, O_WRONLY | O_CREAT | O_APPEND, 0644);
            if(fd_out < 0){
...
void funcao_pipe(char *argumentos[], int contador_de_argumentos, Tarefas cadastradas[], int quantidade_de_tarefas){
    ...
    for(int i = 2; i < contador_de_argumentos; i++){
        ...
        for(int j = 0; j < cadastradas[indice].quantidade_de_argumentos; j++){
            argv_exec[j + 1] = cadastradas[indice].argumento[j];
        }
        argv_exec[j + 1] = NULL;
        ...
        if(pid < 0){
            ...
            if(ordem_da_tarefa > 0){
                dup2(pipe[ordem_da_tarefa - 1][0], 0);
            }

            if(ordem_da_tarefa < quantidade_de_pipe){
                dup2(pipe[ordem_da_tarefa][1], 1);
            }
...
int ler_linha(char *linha, Tarefas cadastradas[], int *quantidade_de_tarefas){
    ...
    if(argc == 1){
        ...
        while(1){
            ...
            if(strcmp(argumentos[0], "task") == 0){
                ...
                } else if (strcmp(argumentos[1], "funcao_sequential") == 0){
                    funcao_sequential(argumentos, contador_de_argumentos, cadastradas, quantidade_de_tarefas);

                } else if (strcmp(argumentos[1], "funcao_parallel") == 0){
                    funcao_parallel(argumentos, contador_de_argumentos, cadastradas, quantidade_de_tarefas);
        ...
        while(fgets(linha, sizeof(linha), arquivo) != NULL){
            ...
            int acabou_a_linha = ler_linha(linha, cadastradas, &quantidade_de_tarefas);
